...
subroutine history
  ...
  real(dp), dimension(ndim) :: meanB, rmsB, rmsv
  real(dp), dimension(:), allocatable       :: tmp
  real(dp), dimension(:,:,:,:), allocatable :: loc_var
  real(dp), dimension(:,:,:), allocatable   :: loc_var_y_mean
  ...
  allocate(loc_var_y_mean(iu1:iu2,ku1:ku2,nvar_mean))

  !$OMP PARALLEL WORKSHARE
  loc_var(:,:,:,1) = uin(:,:,:,ir)
  loc_var(:,:,:,2) = uin(:,:,:,iu)/uin(:,:,:,ir)
  loc_var(:,:,:,3) = uin(:,:,:,iv)/uin(:,:,:,ir)
  !$OMP END PARALLEL WORKSHARE
  
  call compute_y_mean(loc_var, loc_var_y_mean, nvar_mean)
  deallocate(loc_var)
  ...
  rmsB    = zero; rmsv     = zero
  !$OMP PARALLEL DO REDUCTION(+: mass, magp, maxwell, meanB, rmsB, rmsv) &
  !$OMP REDUCTION(+:reynolds, divB) SCHEDULE(RUNTIME)
  do k = 1, nz
     do j = 1, ny
        do i = 1, nx
           ...
           maxwell = maxwell &
                ...
                & *(uin(i,j,k,iB) + uin(i,j+1,k,iB))

           meanB(1) = meanB(1) + uin(i,j,k,iA)
           meanB(2) = meanB(2) + uin(i,j,k,iB)
           meanB(3) = meanB(3) + uin(i,j,k,iC)
           
           rmsB(1)  = rmsB(1) + uin(i,j,k,iA)**2
           rmsB(2)  = rmsB(2) + uin(i,j,k,iB)**2
           rmsB(3)  = rmsB(3) + uin(i,j,k,iC)**2

           rmsv(1)  = rmsv(1) + (uin(i,j,k,iu)/uin(i,j,k,ir))**2
           rmsv(2)  = rmsv(2) + (uin(i,j,k,iv)/uin(i,j,k,ir))**2
           rmsv(3)  = rmsv(3) + (uin(i,j,k,iw)/uin(i,j,k,ir))**2

           reynolds = reynolds + uin(i,j,k,ir)*dtau &
                & *(uin(i,j,k,iu)/uin(i,j,k,ir) - loc_var_y_mean(i,k,2)) &
                & *(uin(i,j,k,iv)/uin(i,j,k,ir) - loc_var_y_mean(i,k,3))
  ...
  call vol_average(rmsB, 3)
  call vol_average(rmsv, 3)
#endif
  ...
  if (mype == 0) then
     if (time == zero) then
        ...
        if (fexist) then
           ...
           open(unit=2, file=filename, status="unknown")
           write(2, "('# ', A11, 1X, A12, 1X, 15(A14,1X))") "time", "dt" &
                & , "mass", "maxwell", "reynolds", "max+rey", "magp", "meanBx" &
                & , "meanBy", "meanBz", "rmsBx", "rmsBy", "rmsBz", "rmsvx" &
                & , "rmsvy", "rmsvz", "divB"
        endif
     endif
     hist_format = '(1X, 1PE12.5, 1X, 1PE12.5, 1X, 15(E14.5,1X))'
     write(2, hist_format) time, dt, mass, maxwell, reynolds, maxwell+reynolds &
          & , magp, meanB(1), meanB(2), meanB(3), rmsB(1), rmsB(2), rmsB(3) &
          & , rmsv(1), rmsv(2), rmsv(3), divB
     close(2)
  endif
#endif
...
subroutine compute_y_mean(quant, mean_quant, nquant)
  ...
  integer, intent(in) :: nquant
  real(dp), dimension(iu1:iu2,ju1:ju2,ku1:ku2,nquant), intent(in) :: quant
  real(dp), dimension(iu1:iu2,ku1:ku2,nquant), intent(out) :: mean_quant
  real(dp), dimension(:,:,:), allocatable :: mean_quant_local
  real(dp) :: mean_quant_loc
  integer :: i, j, k, ivar
#if MPI == 1
...
subroutine compute_z_mean(quant, mean_quant, nquant)
  ...
  integer, intent(in) :: nquant
  real(dp), dimension(iu1:iu2,ju1:ju2,ku1:ku2,nquant), intent(in) :: quant
  real(dp), dimension(iu1:iu2,ju1:ju2,nquant), intent(out) :: mean_quant
  real(dp), dimension(:,:,:), allocatable :: mean_quant_local
  real(dp) :: mean_quant_loc
  integer :: i, j, k, ivar
#if MPI == 1
